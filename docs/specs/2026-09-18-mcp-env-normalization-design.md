...
已完成共享归一化函数，五条返回路径统一返回 McpServerRow[]，并新增 15 个参数化回归测试，覆盖 JSON 解码、结构保留、非法输入拒绝、错误信息脱敏及原始事件不变。

验证结果：MCP 桥接、WorkspaceHub 和 bridge-client 定向测试共 99 项全部通过；桌面主进程及 renderer 的 TypeScript 类型检查通过；代码变更的 git diff --check 通过。未创建 git 提交。
